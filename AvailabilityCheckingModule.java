import java.util.*;
 ...
 public class AvailabilityCheckingModule {
    ...
    public AvailabilityCheckingModule(List<Vehicle> vehicles, List<PickupSpot> pickupSpots) {
    ...
    public List<Vehicle> getVehiclesFromPickupSpots(List<PickupSpot> sortedOrders, List<Vehicle> vehicles) {
        ...
        int flag=0;
                for(int n=0;n<sortedOrders.size();n++)
                {
                    for( int m=0;m<vehicles.size();m++)
                       {  if((sortedOrders.get(n).pickup == vehicles.get(m).loc)&&(vehicles.get(m).available==true))
    ...
    public int getDistance(PickupSpot a, PickupSpot b) {
      int ans = a.getName() - b.getName();
        ...
        public List<PickupSpot> bubbleSortAscendingOrderInDistanceList(List<PickupSpot> pickupSpots, List<Integer> distance)
         {
